...
module.exports = function (app, options) {
    ...
    //console.log('processed file: ' + path.join(options.basePathJson, options.routeName + '.json'));
    ...
    for (uri in json) {
        ...
        if (_.isString(params)) {
            ...
            // set default action if not in params
            if (_.isUndefined(params.action)) params.action = action;

            // set custom controller dir

            if (_.isString(params.controllerPath)) {
                controllerPath = path.join(options.processdir, params.controllerPath);
            }
        ...
        // add controller
        var routes = require(path.join(controllerPath, controller + '.js'));
