...
module.exports = function (app, options) {
    ...
    let jsonFileName = path.join(options.basePathJson, options.routeName + '.json')
    ...
    function displayRoutes(pattern, verb, status, err, corsRoute) {
        ...
        // set cors status
        let corsStatus = ' - \x1b[0mCORS: \x1b[32mOFF\x1b[0m';
     ...
     * @returns {*}
     */
    function parseRoute(routeParams = null) {
    ...
    function getMiddleware(input) {
        ...
        let mdlw = getAllMiddleWare(input);
    ...
    function parseMiddleware(input) {
        ...
        if (parts.length != 2)
            error(uri, "malformed json middleware");

        let middlewareFile = getMiddlewareFile(parts[0]);
        let middleware = require(middlewareFile);
    ...
    function checkGlobalVars(json) {
        return json.hasOwnProperty('GLOBAL');
    }
    ...
    function setFileGlobalParams(json) {
        ...
        if (checkGlobalVars(json)) {
            ...
            if (json.GLOBAL.hasOwnProperty('policy')) {
                ...
                if (Array.isArray(json.GLOBAL.policy)) {
