    ...
    function addRoute(middleware, verb, pattern, handler) {
        ...
        try {
            ...
            if (options.displayRoute) {
                displayRoutes(pattern, verb, 1, null);
            }

        } catch (ex) {

            displayRoutes(pattern, verb, 0, ex);
/*
    ...
    function displayRoutes(pattern, verb, status, err) {



        if (status) {
            console.log('\x1b[32m* \x1b[0mLoad route: \x1b[34m' + pattern + ' - \x1b[35m' + verb.toUpperCase()+'\x1b[0m - file:\x1b[34m'+options.routeName + '\x1b[32m - OK ' + '\x1b[0m');
        } else {
            console.log('\x1b[31m* \x1b[0mLoad route: \x1b[34m' + pattern + ' - \x1b[35m' + verb.toUpperCase()+'\x1b[0m - file:\x1b[34m'+options.routeName + '\x1b[31m - KO ' + '\x1b[0m');
            console.log('               ' + err);
